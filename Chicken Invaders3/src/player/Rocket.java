...
public class Rocket implements Animatable, Serializable {
	...
	public Rocket(int x, int y, String i) {
		this.x = x;
		this.y = y;
		imagePath = i;
	...
	public void collapse(Wave wave) {
		...
		for (GeneralGroups g : wave.groups) {
			Collision.rocketSkullCollapse(this, g.skulls);
		}
		Collision.skullDropCollision(this, wave.drops);
		if (wave.monster != null)
			Collision.rocketGhoolCollapse(this, wave.monster.ghools);
	}
...
//		this.collapse();
	...
	public void constantShelik(double x, double y, int fire_type, int fire_stage) {
		if (System.currentTimeMillis() - shelik_time > fire_delay && shooting && dagh == false) {
